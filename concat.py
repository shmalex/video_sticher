...
if __name__ == "__main__":
    ...
    while True:
        ...
        results = service.files().list(
            ...
            q=f"'1vRpM2GnlTqRbpsqnOGJBp18QLKdyk_Gh' in parents  and createdTime > '{search_dt}' " +
...
#         print('page_token',page_token)
        ...
        with tqdm.tqdm(items) as tq:
            for item in tq:
                tq.set_description(desc=u'{2} {0} ({1})'.format(item['name'], item['id'], item['createdTime']), refresh=True)
                file_id = item['id']
    ...
    created = {get_date(ren_file):os.path.basename(ren_file) for ren_file in ren_files}
    ...
    for i, file in enumerate(tqdm.tqdm(files, desc='coping')):
        ...
        if title_text in created:
            if created[title_text] == file_name: continue
        exif = img.info['exif']
...
#         shutil.copyfile(file, os.path.join(dest_dir, f'{i:0{n}}.jpg'))
    ...
    print("Done renaming")
    print("Running concatination")
    
    ##################### Create Video

    target_dir = os.path.abspath(os.path.join(dest_dir, f"%{n}d.jpg"))
    video_dir = os.path.abspath(os.path.join(dest_dir, '..', f'video_fps{fps}.mp4'))
    print('Concat Target path', target_dir)
    bashCommand = f"snap run ffmpeg -y -vsync 0 -hwaccel cuda -hwaccel_output_format cuda -r {fps} -y -f image2 -i {target_dir}  -tune hq -b:v 20M -bufsize 20M -maxrate 20M -c:v h264_nvenc -crf 30 {video_dir}"

    #     bashCommand = f"ffmpeg -r {fps} -y -f image2 -i {target_dir} -c:v libx264 -crf 30 {video_dir}"
